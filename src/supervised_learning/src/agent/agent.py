...
class Agent(nn.Module):
    ...
    def forward(self, x):
        ...
        # traj = self.plan_net.planning(x, self.mode)
    ...
    def path_planning(self, x):
        ...
        # pdb.set_trace()
        traj, traj1 = self.plan_net(x, self.mode)
        # traj = self.plan_net.planning(x, self.mode)
        # traj = self.plan_net.planning_v0(x, self.mode)
    ...
    def learn(self, traj, traj_ineq, data, train=True):
        """calculate self-supervised learning loss
        Args:
            traj (b,l,2): _description_
            data (b,360+2+15*2): lidar_r(0:360),angular(361:362),goal_dir(363:364),init_w(364:365),cur_pose_w(366:367),goal_pose_r(368:369)
        """
        loss = 0
        ...
        exp_traj = torch.zeros(self.batch_size, 15, 2).to(self.device)
        exp_traj[:,:,0] = data[:,-30:-15]  #(b,15)
        exp_traj[:,:,1] = data[:,-15:]  #(b,15)
        data = data[:,:-30]  #(b,:-30)
        ...
        traj_dist = torch.linalg.norm(traj - data[:, None, -2:], dim = -1)  #(b,l)
        
        # dynamical distance loss
        # goal_reached = torch.ones_like(traj_dist[:,0])  #(b,)
        loss_dist = 0.
        for i in range(self.length):
            loss_dist += torch.linalg.norm(traj[:,i] - exp_traj[:,i], dim = -1).mean()
        ...
        loss_col = 0
        # pdb.set_trace()
        err_dist = torch.cdist(traj, rad_points)  #(b,l,360)
        pre_dist = err_dist[:,0].min(-1).values  #(b,)
        for i in range(1, self.length):
            loss_col += self.act_cbf(- (err_dist[:,i].min(-1).values - pre_dist + self.cbf_epsilon * err_dist[:,i].min(-1).values)).mean()
            pre_dist = err_dist[:,i].min(-1).values
        
        # evaluate trajectory smoothness
        angle_loss = 0.
        pre_angle = torch.atan2(traj[:,i,1], traj[:,i-1,0] + self.sm_epsilon)
        goal_not_reached = torch.ones_like(traj_dist[:,0])  #(b,)
        # check_close = torch.ones((traj_dist.shape[0],self.length),device=self.device)  #(b,l)
        for i in range(1,self.length):
            goal_not_reached = goal_not_reached * torch.gt(traj_dist[:, i], 0.1).detach()  #(b,)
            cur_x = (traj[:,i,0] - traj[:,i-1,0] + self.sm_epsilon) * ((traj[:,i,0] - traj[:,i-1,0]) > 0).float()
            cur_x += (traj[:,i,0] - traj[:,i-1,0] - self.sm_epsilon) * ((traj[:,i,0] - traj[:,i-1,0]) < 0).float()
            # cur_angle = torch.atan2(traj[:,i,1] - traj[:,i-1,1], traj[:,i,0] -traj[:,i-1,0]+ self.sm_epsilon) * goal_not_reached
            cur_angle = torch.atan2(traj[:,i,1] - traj[:,i-1,1], cur_x) * goal_not_reached
            angle_loss += torch.abs(cur_angle - pre_angle).mean()
            pre_angle = cur_angle
        ...
        if not(train):
            for i in range(360):
                if data[0,i] < 3.5:
                    plt.scatter(rad_points[0,i,0].cpu().numpy(),rad_points[0,i,1].cpu().numpy(), color='blue')
            
            for j in range(self.length):
                plt.scatter(exp_traj[0,j,0].detach().cpu().numpy(), exp_traj[0,j,1].detach().cpu().numpy(), color='purple')
            ...
            for i in range(self.length):
                plt.scatter(traj_ineq[0,i,0].detach().cpu().numpy(), traj_ineq[0,i,1].detach().cpu().numpy(), color='yellow')
        ...
        return loss.detach().cpu().numpy(), distance_loss.detach().cpu().numpy(), collision_loss.detach().cpu().numpy(), smooth_loss.detach().cpu().numpy()
