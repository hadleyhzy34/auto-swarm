...
class Agent(nn.Module):
    def __init__(self, state_size, action_size, config=None, device = torch.device('cpu')):
        super().__init__()
        self.config = config
        ...
        self.sm_epsilon = 1e-6

        # cbf loss
        self.cbf_epsilon = 0.15
    ...
    def learn(self, traj, data, train=True):
        ...
        # # loss_dist = -torch.log(torch.clamp(r2g_min/self.r2g_safe, min = 1e-6, max = 1.)).mean()
        # loss_dist = torch.clamp((self.r2g_safe - r2g_min) / self.r2g_safe, min = 1e-6, max = 1.).mean()
        ...
        # loss_step = self.step_weight * loss_step
        # loss = loss_dist + loss_col + loss_angle + loss_step
