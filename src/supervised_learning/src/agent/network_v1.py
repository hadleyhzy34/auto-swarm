...
import pdb
from agent.attention import Crossformer, Crossformer_v0
...
class Net(nn.Module):
    def __init__(self, state_dim, action_dim, scan_range = 360, length = 10, device = torch.device('cpu')):
        ...
        # self.crossformer = Crossformer_v0(self.feats_dim, self.heads, self.feats_dim, self.feats_dim, device=self.device)
    ...
    def planning(self, s, mode = 'train'):
        batch_size = s.shape[0]
        rad_points = torch.zeros((batch_size, 360, 2),device=self.device)  #(b,360,2)
        # pdb.set_trace()
        ...
        # initial trajectory
        # pdb.set_trace()
        # cur_pos = (s[:,-2:] - s[:,-4:-2])[:,None,:].repeat(1,self.length,1) / self.length  #(b,l,2)
        cur_goal_pose = s[:,360:362]  #(b,2)
        cur_pos = cur_goal_pose[:,None,:].repeat(1,self.length,1) / self.length  #(b,l,2)
        # cur_pos = s[:,None,-4:-2] + cur_pos * torch.arange(1,self.length+1).cuda()[None,:,None].repeat(batch_size,1,1)  #(b,l,2)
        # cur_pos = cur_pos * torch.arange(1,self.length+1).cuda()[None,:,None].repeat(batch_size,1,1)  #(b,l,2)

        if mode == 'train':
            cur_pos_feats = self.crossformer(cur_pos, rad_points, cur_goal_pose)  #(b,l,hd)
            # pdb.set_trace()
            cur_pos = self.traj_mlp(cur_pos_feats)  #(b,l,2)
    ...
    def planning_v0(self, s, mode = 'train'):
        batch_size = s.shape[0]
        rad_points = torch.zeros((batch_size, 360, 2),device=self.device)  #(b,360,2)
        # pdb.set_trace()
        rad_points[:,:,0] = torch.cos((torch.arange(0,360).to(self.device)) * torch.pi / 180) * s[:,0:360]
        ...
        if mode == 'train':
            cur_pos_feats = self.crossformer(cur_pos, rad_points, cur_goal_pose)  #(b,l,hd)
            # pdb.set_trace()
            traj = self.traj_mlp(cur_pos_feats)  #(b,l,2)
    ...
    def forward(self, s, mode='train'):
        ...
        args:
            s: (batch_size, state_dim: 360+2+30)
        return:
