...
class Env():
    ...
    def getState(self, scan):
        ...
        cur_goal_dist = np.linalg.norm(goal_pos)
        
        if cur_goal_dist < 0.1:
    ...
    def step(self, action):
        ...
        self.goal_distance = np.linalg.norm([self.goal_x - self.position.x, self.goal_y - self.position.y])
    ...
    def reset(self):
        while(True):
            ...
            self.goal_reached = False
            self.collided = False
            goal_pose = np.array([self.goal_x - self.position.x, self.goal_y - self.position.y])
            self.goal_distance = np.linalg.norm(goal_pose)
            # self.goal_distance = self.getGoalDistace()
            state, done = self.getState(data)
            # break only if its not done
            if not(done):
                break
    ...
    def random_pts_map(self):
        ...
        # while dist < .4 or dist > self.dist_range:
        while dist < 0.4 or dist > self.dist_range:
