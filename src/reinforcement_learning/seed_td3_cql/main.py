...
def learn(gAgent, shared_buffer:Buffer):
    # writer = SummaryWriter()
    global_step = 0
    # print(f'pointer: {shared_buffer.pointer[0]}, capacity: {shared_buffer.capacity}')
    while(True):
        # print(f'pointer: {shared_buffer.pointer[0]}, capacity: {shared_buffer.capacity}')
        if shared_buffer.pointer[0] >= shared_buffer.capacity:
            # loss = gAgent.learn(shared_buffer)
            # gAgent.learn(shared_buffer)
            # print(f'test if current model is being learned')
            loss, cql_loss, bell_error = gAgent.learn(shared_buffer)
...
if __name__ == '__main__':
    ...
    for rank in range(num_processes+1):
        if rank != num_processes:
            ...
            print(f'process is initialized........................................................................')
        p.start()
        processes.append(p)
